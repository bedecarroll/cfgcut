...
- The release workflow at `.github/workflows/release.yml` is generated by `cargo-dist`; update it via `dist init`/`cargo-dist-version`, not by hand (CI will fail if it drifts).
- For Dependabot GitHub Actions bumps that touch generated release workflow pins, update `dist-workspace.toml` `github-action-commits` first, then keep `.github/workflows/release.yml` aligned and run `dist plan`/`mise run dist-plan`. Reverting only the generated workflow can make CI green while leaving Dependabot able to reopen the same update.
